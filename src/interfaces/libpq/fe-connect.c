...
//
const QUIC_REGISTRATION_CONFIG RegConfig = { "libpq", QUIC_EXECUTION_PROFILE_LOW_LATENCY };
...
ClientStreamCallback(
    ...
    case QUIC_STREAM_EVENT_START_COMPLETE:
      printf("[strm][%p] Stream Start Complete: %ld\n", Stream, Event->START_COMPLETE.ID);
				  ...
				  int expand_dbname)
{
	...
	if (QUIC_FAILED(Status = MsQuicOpen2(&MsQuic))) {
		...
		libpq_append_conn_error(conn, "could not open msquic. error: 0x%x", Status);
    }
    ...
    if (QUIC_FAILED(Status = MsQuic->RegistrationOpen(&RegConfig, &Registration))) {
		...
		conn->status = CONNECTION_BAD;
		libpq_append_conn_error(conn, "could not open msquic registration. error: 0x%x", Status);
    }
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->ConnectionOpen(Registration, ClientConnectionCallback, NULL, &conn->ClientConnection))) {
	...
	if (getenv("SSLKEYLOGFILE") != NULL) {
		if (QUIC_FAILED(Status = MsQuic->SetParam(conn->ClientConnection, QUIC_PARAM_CONN_TLS_SECRETS, sizeof(ClientSecrets), &ClientSecrets))) {
			printf("SetParam(QUIC_PARAM_CONN_TLS_SECRETS) failed, 0x%x!\n", Status);
		}
	}
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->ConnectionStart(conn->ClientConnection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, Target, UdpPort))) {
	...
	// TODO: Preciso colocar essas estruturas dentro do PGConn e permitir que as streams
	// sejam usadas la do pqSendSome

    // Waits for the client Connection.
	// Empty while here does not work, maybe because we need to follow some
	// interruption pattern
    while(ClientConnection == NULL){ printf("Waiting for connection..."); };

    //
    // Create/allocate a new bidirectional stream. The stream is just allocated
    // and no QUIC stream identifier is assigned until it's started.
    //
    if (QUIC_FAILED(Status = MsQuic->StreamOpen(conn->ClientConnection, QUIC_STREAM_OPEN_FLAG_NONE, ClientStreamCallback, NULL, &conn->Stream))) {
        printf("StreamOpen failed, 0x%x!\n", Status);
        MsQuic->ConnectionShutdown(conn->ClientConnection, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 0);
		conn->status = CONNECTION_BAD;
  }
  ...
  printf("[strm][%p] Starting...\n", conn->Stream);
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->StreamStart(conn->Stream, QUIC_STREAM_START_FLAG_NONE))) {
        printf("StreamStart failed, 0x%x!\n", Status);
        MsQuic->StreamClose(conn->Stream);
